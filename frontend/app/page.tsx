    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          ...
          <div className="text-sm mt-1">registered members <Link href="/staff" className="underline">manage →</Link></div>
        </div>
        <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-5">
          <div className="text-xs uppercase tracking-widest text-zinc-500 mb-2">Upcoming Bookings</div>
          <div className="text-2xl font-semibold">{bookings.length}</div>
          <div className="text-sm mt-1">recent activity <Link href="/bookings" className="underline">view all →</Link></div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          <div className="flex items-baseline justify-between mb-3 px-1">
            ...
            <Link href="/staff" className="text-sm text-zinc-500 hover:text-black dark:hover:text-white">See all →</Link>
          </div>
        ...
        <div>
          <div className="flex items-baseline justify-between mb-3 px-1">
            ...
            <Link href="/bookings" className="text-sm text-zinc-500 hover:text-black dark:hover:text-white">See all →</Link>
          </div>
          <div className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
            {loading ? (
              <div className="p-8 text-center text-sm text-zinc-400">Loading...</div>
              ...
              <div className="p-8 text-center text-sm text-zinc-500">
                No bookings yet. <Link href="/bookings" className="underline">Create one</Link>.
              </div>
            ) : (
              <ul className="divide-y divide-zinc-100 dark:divide-zinc-800 text-sm">
                {bookings.map((b) => (
                  <li key={b.id} className="px-5 py-3 flex justify-between gap-4">
                    <div>
                      <span className="font-medium">{b.client_name}</span>
                      <span className="text-zinc-400"> with </span>
                      <span>{b.staff_name}</span>
                    </div>
                    <div className="text-right font-mono text-xs text-zinc-500 whitespace-nowrap">
